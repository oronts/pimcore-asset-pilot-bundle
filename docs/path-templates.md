...
| `first_of` | `{{ items\|first_of('key', 'default') }}` | Property from the first item only, which must be an object (read via a `getKey` getter, a public `get`/`is`/`has` accessor method, or a public property); a non-object first entry, e.g. an associative array, yields the fallback. Optional second arg sets the fallback (default `'unknown'`) |
