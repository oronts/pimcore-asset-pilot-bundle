...
On top of these bundle-level permissions, the destructive operations (unused-asset delete and move, audit revert, and duplicate-merge copy disposition) also enforce Pimcore's native per-asset workspace ACL: each asset is checked with `isAllowed('delete')` / `isAllowed('publish')` for the current user, so an Operate or Admin user still cannot act on an asset outside their workspace permissions. CLI runs (e.g. `asset-pilot:cleanup-unused`) are treated as a trusted system context and are not workspace-gated.
