        ...
        return (
          <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ flex: 1, background: '#f5f5f5', borderRadius: 4, height: 16, overflow: 'hidden' }}>
              <div style={{ width: `${(v / max) * 100}%`, background: item.color ?? '#1677ff', height: '100%', borderRadius: 4, minWidth: v > 0 ? 2 : 0 }} />
            </div>
            <span style={{ width: 56, textAlign: 'right', fontSize: 12, fontWeight: 600, color: '#1a1a1a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={formatValue != null ? formatValue(item.value) : String(item.value)}>{formatValue != null ? formatValue(item.value) : item.value}</span>
          </div>
        )
      })}
    </div>
